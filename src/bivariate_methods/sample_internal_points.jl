...
Samples `num_points` internal points of existing bivariate profiles that meet the requirements of [`PlaceholderLikelihood.desired_df_subset`](@ref) (see Keyword Arguments). For each bivariate profile that meets these requirements it creates a 2D polygon hull from it's set of boundary and internal points (method dependent) using `hullmethod` and samples points from the hull homogeneously until `num_points` are found, rejecting any that are not inside the log-likelihood threshold at that `confidence_level` and `profile_type`. Modifies `model` in place, with sampled internal points appended to the internal points field of each [`BivariateConfidenceStruct`](@ref).
...
# Keyword Arguments
- `confidence_levels`: a vector of confidence levels. If empty, all confidence levels of bivariate profiles will be considered for finding interval points. Otherwise, only confidence levels of bivariate profiles in `confidence_levels` will be considered. Default is `Float64[]` (any confidence level).
- `profile_types`: a vector of `AbstractProfileType` structs. If empty, all profile types of bivariate profiles are considered. Otherwise, only bivariate profiles with matching profile types will be considered. Default is `AbstractProfileType[]` (any profile type).
- `methods`: a vector of `AbstractBivariateMethod` structs. If empty all methods used to find bivariate profiles are considered. Otherwise, only bivariate profiles with matching method types will be considered (struct arguments do not need to be the same). Default is `AbstractBivariateMethod[]` (any bivariate method).
...
- `proportion_of_predictions_to_keep`: . Default is `1.0`.
