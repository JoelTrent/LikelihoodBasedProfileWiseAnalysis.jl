...
Use the true log-likelihood function for confidence profile evaluation. The methods [`IterativeBoundaryMethod`](@ref) and [`RadialRandomMethod`](@ref) are recommended for use with this profile type.
...
Use an ellipse approximation of the log-likelihood function centred at the MLE with use of parameter bounds for confidence profile evaluation. The method [`RadialMLEMethod`](@ref) is recommended for use with this profile type.
