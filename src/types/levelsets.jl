...
- `internal_points`: a [`PointsAndLogLikelihood`](@ref) struct containing points and their corresponding log-likelihood values that were found during a method, if `save_internal_points=true`. Default is an empty [`PointsAndLogLikelihood`](@ref) struct (used if `save_internal_points=false`).
...
Struct that stores samples produced by an [`AbstractSampleType`](@ref) that are within the confidence boundary of `sample_dimension` interest parameters at a given confidence level, with their corresponding log-likelihood values.
