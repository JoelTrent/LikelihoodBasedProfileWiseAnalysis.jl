...
Struct containing the core information required to define a [`LikelihoodModel`](@ref). For additional information on parameters (where repeated), see [`initialise_LikelihoodModel`](@ref).
...
Mutable struct containing all the information required to compute profiles, samples and predictions. Created by [`initialise_LikelihoodModel`](@ref).
...
- `uni_profile_row_exists`: a dictionary containing information on whether a row in `uni_profiles_df` exists for a given combination of interest parameter, degrees of freedom, [`AbstractProfileType`](@ref) and confidence level. If it does exist, the value of `uni_profile_row_exists[(θi, dof, profile_type)][confidence_level]` value will be the row index in `uni_profiles_df`, otherwise it will be `0`.
