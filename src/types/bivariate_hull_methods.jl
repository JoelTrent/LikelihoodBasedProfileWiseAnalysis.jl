...
Constructs a 2D polygon hull to sample internal points from by applying a convex hull algorithm from [Meshes.jl](https://docs.juliahub.com/Meshes/FuRcu/0.18.2/algorithms/hulls.html#Meshes.GrahamScan) on the collection of points given by both the boundary and saved internal points. 
...
This method has the ability to create a more accurate representation of the boundary than [`MPPHullMethod`](@ref), but because of it's heuristic nature this is not guaranteed. However, it should be a more accurate representation of the boundary than [`ConvexHullMethod`](@ref) for non-convex boundaries (concave boundaries). The representation in the worst case where the heuristic does not return a good representation of the boundary despite a good initial point cloud, will be signficantly worse than the [`ConvexHullMethod`](@ref) and likely worse than the [`MPPHullMethod`](@ref).
